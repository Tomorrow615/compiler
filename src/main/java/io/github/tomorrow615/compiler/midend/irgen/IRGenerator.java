...
public class IRGenerator {
    ...
    public IRGenerator(CompUnitNode astRoot, List<SymbolTable> allScopes) {
        ...
        // 传入 'this' 以便 StmtGenerator 在处理 Block 时能回调 visitBlock
    ...
    private void declareIOFunctions() {
        ...
        context.getIoFunctions().put("getint", getintFunc);
        Symbol getintSym = context.getCurrentMetadataScope().lookup("getint");
        if (getintSym instanceof FuncSymbol) {
            ((FuncSymbol) getintSym).setLlvmValue(getintFunc);
            context.getCurrentScope().addSymbol(getintSym);
        }
        ...
        context.getIoFunctions().put("putint", putintFunc);
        Symbol putintSym = context.getCurrentMetadataScope().lookup("putint");
        if (putintSym != null) context.getCurrentScope().addSymbol(putintSym);
        ...
        context.getIoFunctions().put("putch", putchFunc);
        Symbol putchSym = context.getCurrentMetadataScope().lookup("putch");
        if (putchSym != null) context.getCurrentScope().addSymbol(putchSym);
        ...
        context.getModule().addFunction(putstrFunc);
        context.getIoFunctions().put("putstr", putstrFunc);
        Symbol printfSym = context.getCurrentMetadataScope().lookup("printf");
        if (printfSym instanceof FuncSymbol) {
            ((FuncSymbol) printfSym).setLlvmValue(putstrFunc);
            context.getCurrentScope().addSymbol(printfSym);
        }
    }
    ...
    public void visitBlock(BlockNode node) {
        ...
        for (BlockItemNode item : node.getBlockItems()) {
            if (builder.getCurrentBlock().hasTerminator()) break;

            if (item instanceof DeclNode d) {
                visitDecl(d); // 声明：自己处理
            } else if (item instanceof StmtNode s) {
                stmtGen.visitStmt(s); // 语句：分发给 StmtGenerator
            }
        }
    }
    ...
    public void visitDecl(DeclNode node) {
        if (node instanceof ConstDeclNode c) visitConstDecl(c);
        else if (node instanceof VarDeclNode v) visitVarDecl(v);
    }

    private void visitConstDecl(ConstDeclNode node) {
        if (context.getCurrentFunction() == null) visitGlobalConstDecl(node);
        else visitLocalConstDecl(node);
    }

    private void visitVarDecl(VarDeclNode node) {
        if (node.isStatic()) visitStaticLocalVarDecl(node); // 静态局部变量当做全局处理逻辑类似
        else if (context.getCurrentFunction() == null) visitGlobalVarDecl(node);
        else visitLocalVarDecl(node);
    }
    ...
    private void visitStaticLocalVarDecl(VarDeclNode node) {
        for (VarDefNode def : node.getVarDefs()) {
            ValueSymbol symbol = (ValueSymbol) context.getCurrentMetadataScope().lookup(def.getIdent().getText());
            // 生成唯一名称
            String uniqueName = context.getNextLabel("@" + context.getCurrentFunction().getName().substring(1) + "." + symbol.getName());
            // 逻辑与全局变量一致，但使用唯一名
    ...
    private void visitLocalConstDecl(ConstDeclNode node) {
        ...
        for (ConstDefNode def : node.getConstDefs()) {
            ...
            if (symbol.getDimension() > 0) {
                ...
                if (initVal != null) builder.createStore(initVal, ptr);
                // 编译期求值
                Constant cVal = evalConstInit(def.getConstInitVal());
                if (cVal instanceof ConstantInt ci) symbol.setConstValue(ci.getValue());
            }
            context.getCurrentScope().addSymbol(symbol);
        }
    }
        ...
        if (symbol.getDimension() > 0) {
            ...
            if (exps != null) {
                for (ASTNode exp : exps) {
                    ...
                    ConstExpNode constExp = (exp instanceof ConstExpNode) ? (ConstExpNode) exp : new ConstExpNode(((ExpNode)exp).getAddExp());
            ...
            initializer = val;
            if (isConst && val instanceof ConstantInt ci) symbol.setConstValue(ci.getValue());
        }
    ...
    private void handleStaticInit(ValueSymbol symbol, ASTNode initValNode, String uniqueName) {
        ...
        if (symbol.getDimension() > 0) {
            ...
            while (initValues.size() < size) initValues.add(new ConstantInt(0));
            initializer = new ConstantArray(type, initValues);
        } else {
            type = IntegerType.i32;
    ...
    private void handleLocalArrayInit(Value ptr, ValueSymbol symbol, ASTNode initValNode, boolean isConst) {
        ...
        for (int i = 0; i < exps.size(); i++) {
            ...
            // 运行时 Store
            Value val;
            if (isConst) val = exprGen.visitConstExp((ConstExpNode) exp);
            else val = exprGen.visitExpression(exp);
    ...
    private Constant evalInitVal(ASTNode node) {
        if (node == null) return new ConstantInt(0);
    ...
    private List<? extends ASTNode> getArrayInitExps(ASTNode node) {
        if (node instanceof ConstInitValNode n && n.getType() == ConstInitValNode.Type.ARRAY) return n.getArrayInit();
        if (node instanceof InitValNode n && n.getType() == InitValNode.Type.ARRAY) return n.getArrayInit();
        return null;
    }
}